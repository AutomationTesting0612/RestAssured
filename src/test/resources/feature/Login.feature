...
#      | 1  |george.bluth@reqres.in|
  ...
  Scenario Outline: Reqres POST API test
    ...
    Then the new user must be created with name as "<username>", "<statusCode>"
  ...
  @SmokeTest
  Scenario Outline: Reqres put API test

    Given the valid endpoint with payload to update user
    When the request is send to the server for PUT operator "<id>"
    Then the new user must be created with name as "<username>", "<statusCode>"

    Examples:
      |username|id   |statusCode |
      | Test   |1    |200        |
